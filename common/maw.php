...
if (!(in_array($lang,$lang_array))) {$lang="en";}
...
// gettext
if ($lang=="cs") {$langl="cs_CZ"; $locale_file = "cs_CZ";}
if ($lang=="en") {$langl="en_US"; $locale_file = "en_US";}
if ($lang=="pl") {$langl="pl_PL"; $locale_file = "pl_PL";}
if ($lang=="ca") {$langl="ca_ES"; $locale_file = "ca_ES";}
if ($lang=="fr") {$langl="fr_FR"; $locale_file = "fr_FR";}
if ($lang=="zh") {$langl="zh_CN"; $locale_file = "zh_CN";}
if ($lang=="ru") {$langl="ru_RU"; $locale_file = "ru_RU";}
if ($lang=="de") {$langl="de_DE"; $locale_file = "de_DE";}
if ($lang=="it") {$langl="it_IT"; $locale_file = "it_IT";}
if ($lang=="uk") {$langl="uk_UA"; $locale_file = "uk_UA";}
if ($lang=="es") {$langl="es_ES"; $locale_file = "es_ES";}
...
if (file_exists('../common/mawconfig.php')) {require ('../common/mawconfig.php');}
...
function maw_html_head($message="")
{
  ...
  if ($mawhead_used==1) { echo ('<span style="display:none;"> '.$message.' </span>'); return; }
  { 
...
function maw_howto()
{
  ...
  if ($tempvar!=' ')
    {
      if (strlen($tempvar)==1) {echo sprintf("<span class='bold'>".__('variable %s')."<span>",$variables);}
      else {echo sprintf("<span class='bold'>".__('variables %s')."<span>",$tempvar);}
    }
  else
    {
...
function save_log ($maw_string,$soubor)
{
  ...
  if (stristr($maw_URI,'?')) {$temp=" <a href=\"../..".$maw_URI."&amp;internal=nosavelog\">link</a>";}
  if (((!(stristr($maw_URI,"nosavelog")))&&(!(stristr($maw_URI,"format=png"))))||(stristr($soubor,"too_fast_error")))
    {
	if ($soubor=="errors") {$maw_string=$scriptname." ".$maw_string;}
...
function highlight_parentheses($maw_string, $error=false)
{
  ...
  $parlevel=0;
  $output="";
  if ($error) {$err="err";} else {$bg="";}
  for ($k = 0; $k < $j; $k++) 
    {
      $char = substr($maw_string, $k, 1);
      if ($char=="(") 
	{$parlevel++; $temp=$parlevel%4; $output=$output."<span class='color$temp$err'>(";}
      elseif ($char==")") 
	{$parlevel--;$output=$output.")</span>";}
      else 
	{$output=$output."$char";}
    }
...
function check_math_errors($maw_string)
{
  ...
  $badMaximaFunctions = Array('erf', 'pow');
...
echo json_encode(array('data' => $newfile, 'file' => basename($file) ));
}
...
$formconv_repl_input=array();
$formconv_repl_output=array();
  ...
  if (!(preg_match('~^-?[0-9]+\.?[0-9]*$~',$vstup)) )
    {
      ...
      if (!($decfieldname=="")) {echo ("<br>".sprintf(__("Invalid input in the field %s."),"<b>".$decfieldname."</b>"));}
      die();
    }
  return($vstup);
}
...
function check_for_errors($output,$string,$file,$msg="")
{
  ...
  {
    if ($msg=="") {$msg=__("An error occurred when processing your input.<br>Check your formulas (perhaps using Preview button) and report the problem if you think that your input is correct and should be processed without any error.");}
    maw_html_head("MAWerror");
