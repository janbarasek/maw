...
$ymin=$_REQUEST["ymin"];
$ymax=$_REQUEST["ymax"];
if (!($xmin=="")) {$xmin=check_decimal($xmin,"xmin");}
if (!($xmax=="")) {$xmax=check_decimal($xmax,"xmax");}
if (!($ymin=="")) {$ymin=check_decimal($ymin,"ymin");}
if (!($ymax=="")) {$ymax=check_decimal($ymax,"ymax");}
...
\end{document}';
  ...
  $tempHTML= $funkceHTML;
  if ($fbHTML != $faHTML) { $tempHTML = $tempHTML." = ".$fbHTML; }
  ...
  if ($parityHTML=="odd") {$temp=_("The function is odd.");}
  if ($parityHTML=="even") {$temp=_("The function is even.");}
  ...
  if ($stacHTML == ' '){$temp = __("Maxima failed to find stationary points. They probably do not exist.") ;}
  else { $temp = sprintf("<p>%s :&nbsp;&nbsp;&nbsp; $\\displaystyle %s $ </p>",__("Stationary points"),$stacHTML);}	   
  ...
  if ($kritHTML == ' '){$temp = __("Maxima failed to find critical points. They probably do not exist.") ;}
  else { $temp = sprintf("<p>%s :&nbsp;&nbsp;&nbsp; $\\displaystyle %s $ </p>",__("Critical points"),$kritHTML);}	   
  $htmlFile = $htmlFile.$temp;
  $htmlFile = $htmlFile."</div></div>";
...
/* ---------------------------------------------------------------------*/
  ...
  if ($lostsolsHTML != "") {$lostsolsHTML = __("Maxima is using arc-trig functions to get a solution. Some solutions will be lost.");}
