...
if ($naturallog=="1") {$logbase="exp(1)";$logbasegnuplot="exp(1)";}
...
$deformace=0;
if (stristr($vystupk,"nasobek")){$deformace=1;}
...
if (($out=="html") and ($funkcetex!=$funkcetexorig) )
  {
    ...
    if (($vodpos==0) && ($svpos==0)) {$style=__("bold");} else {$style = __("red");}
      if ($deformace){echo '<li>'.sprintf(__("Graph deformation: we draw function %s."),"&nbsp;&nbsp;\$y=".myformconv_replacements($funkcetexnas)."\$&nbsp;&nbsp;",$style).'</li>';}
...
if ($mrizka == "on") { fwrite($souborobr,"set grid\n"); }
...
if ($vas!=""){fwrite($souborobr,"unset arrow 2\n");}
if ($svas!=""){fwrite($souborobr,"unset arrow 1\n");}
...
if (($out=="gif") or ($out=="pdf"))
  {
	...
	if ($j<10) {$jj="00".$j;} elseif ($j<100) {$jj="0".$j;} else {$jj=$j;}
...
$j=$j+1;
	...
	for ($i=0;$i<=20;$i++) 
	  {
	    if ($svas !=""){   
	      if ($i>0) {fwrite($souborobr,"unset arrow 1\n");}
...
$j=$j+1;
	...
	for ($i=0;$i<=20;$i++) 
	  {
	    if ($vas !=""){   
	      if ($i>0) {fwrite($souborobr,"unset arrow 2\n");}
...
\parindent 0 pt';
  ...
  $TeXtempa="";   $TeXtempb="";
  if ($lang=="zh") {$TeXtempa='\begin{CJK*}{UTF8}{gbsn}'; $TeXtempb='\newpage\end{CJK*}';}
  $TeXfileB='
\begin{document}
'.$TeXtempa.'
\MAWhead{%s}
...
';
  ...
  if ($lang=="zh") {$TeXheader=str_replace("\\AtBegin","%\\AtBegin",$TeXheader);}
