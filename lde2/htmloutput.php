...
\begin{aligned}
  ...
  &A'(x) && \left[<?php echo $fundsader; ?>\right]&{}+{}&B'(x)&&\left[<?php echo $fundsbder; ?>\right] &&{}= <?php echo $pravastrana; ?>
\end{aligned}
\]
...
if ($pravastranaexp=="1") {$temp="";} else {$temp=$pravastranaexp;}
...
if ($p!='0') { $tempecho = $tempecho."+ $p \\underbrace{($uprder)}_{y'} ";}
...
if ($p!='0') { $tempecho = $tempecho."+ $p \\underbrace{($uprder)$pravastranaexp}_{y'}"; }
...
else { printf(__('and divide by the common exponential factor %s and add like powers of $x$'),"\$ $pravastranaexp\$");}
