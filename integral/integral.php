...
<a href="http://sourceforge.net/tracker/?group_id=221048&amp;atid=1052162">' . __("Report bug") . '</a><br>';
	$backlink = "";
	if ($oriprom == "x") {
		$backlink = rawurlencode($orifce);
	}
	echo "<a href=\"$mawhtmlhome/index.php?form=integral&lang=$lang&function=$backlink\">" . __("Go home") . "</a>";
	echo "\n </div>";
	if ($jsmath == "on") {
		echo '<SCRIPT type="text/javascript" SRC="../../jsMath/jsMath.js"></SCRIPT>';
	}
	echo '  <noscript> 
...
if (($logarcswitch == "on") && (preg_match('~asinh|acosh|atanh|acoth|asech|acsch~', $fce))) //if ($logarcswitch=="on")
{
...
function maxima_command($prikaz, $opt = "", $matchint = "matchint")
{
	...
	if (!(ereg("I", $vystupTeXarray[1]))) {
		...
		if ($vystupTeXarray[1] != "") {
			$TeXstrings[$vystup[1]] = formconv_replacements($vystupTeXarray[1]);
		}
	}
...
if ($akce == "s") {
	if ((strlen($novapromenna) != 1) or ($novapromenna == $prom) or ($substituce == "") or (ereg($novapromenna, $allprom)) or (!(ereg($novapromenna, $substituce)))) {
		...
		if (!(ereg($novapromenna, $allprom))) {
			$allprom = $allprom . $novapromenna;
		}
	}
}
...
function vypocitej($akcenotused, $fce, $promenna)
{
	...
	if ($akce == "undo") {
		...
		if ($substituce_back != "") {
			$substvztah_back = math_to_tex($substituce_back);
		}
		...
		if (ereg("false", $vsechno)) {
			maw_integral_error();
			die(__("<b>Error. The most general expression which you can remove from the integral is  a nonzero number written with digits or a multiple of constants pi and e.</b> Go back and revise your choice.") . hide_message());
		}
	} elseif ($akce == "pp") {
		$variables = $prom;
		if ($u != "") {
		...
		if (ereg("false", $vypocet)) {
			maw_integral_error();
			die(hide_message() . __("<b>Error. You cannot multiply numerator and denominator by zero or a zero equivalent expression.</b> Go back and revise your choice."));
		}
		if ($rozsiritradcan == "on") {
	...
	if ($akce == "i") {
		...
		if (!($prom == $oriprom)) {
			$komentar = $komentar . " " . __("(remember to return to the original variable)");
		}
		$komentar = $komentar . ":";
	}
...
function substituce($fce, $substituce, $novapromenna)
{
	...
	$opt = "";
	$test = str_replace(" ", "", $substituce);
	if ((ereg("tan\(\(\(1/2\)\*$prom\)\)=$novapromenna", $test)) || (ereg("tan\($prom/2\)=$novapromenna", $test)) || (ereg("tan\(\($prom/2\)\)=$novapromenna", $test))) {
		$opt = "trigexpand";
	}
	if ($kladna == "on") {
	...
	if ($substituce_back_orig == "") {
		...
		if ($backsubst == "") {
			$backsubst = $substituce_back_orig;
...
function integrace($fce, $prom, $usemap = false)
{
	...
	if ((ereg("integrate", $result)) || ($result == "")) {
		...
		if (isset($TeXstrings[$result])) {
			$result = tex_to_html($TeXstrings[$result]);
		}
		if ($prom == 'x') {
			$fce_in_x = $fce;
		} else {
			$fce_in_x = maxima_command("ev($fce,$prom=x)");
		}
...
if ($krok == 1) {
	...
	if ($oriprom == "x") {
		$link = $link . "&function=" . rawurlencode($orifce);
	}
	$link = $link . '">';
...
function convert_numberformula_to_formula($number)
{
	...
	if ($jsmath != "on") {
		$output = "\\usepackage{color}\\color{" . "$texred}\\small";
	}
...
function hint_formula($maximaoutput)
{
	...
	if ($formula_number == 0) {
		return (__("integral of polynomial or constant"));
	}
...
{
	global $allprom, $prom;
	if (!(ereg("t", $allprom . $prom))) {
		return ("t");
	}
	if (!(ereg("p", $allprom . $prom))) {
		return ("p");
	}
	if (!(ereg("q", $allprom . $prom))) {
		return ("q");
	}
	if (!(ereg("r", $allprom . $prom))) {
		return ("r");
	}
	if (!(ereg("s", $allprom . $prom))) {
		return ("s");
	}
	if (!(ereg("g", $allprom . $prom))) {
		return ("g");
	}
	if (!(ereg("h", $allprom . $prom))) {
		return ("h");
	}
...
function convert_degree_to_words($n)
{
	...
	if ($n == 0) {
		...
		return (__("linear polynomial"));
	} elseif ($n == 2) {
		return (__("quadratic polynomial"));
	} elseif ($n == 3) {
		return (__("cubic polynomial"));
	} else {
		return (sprintf(__("%s degree polynomial"), $n));
	}
}
...
', __("integrate terms in sum (mark terms to be integrated, you have to mark at least one term)"), '</b>';
	...
	function scitanec($cislo, $vyraz)
	{
		...
		$tempcheck = "";
		if ((ereg("### testformula", $tempcomp)) || (ereg("### logofdenom", $tempcomp))) {
