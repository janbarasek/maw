...
if (($vars=="r dr dphi")||($vars=="r dphi dr")) {$variables="r|phi";}
...
$vzorce=Array();
function def_tex($string,$def) {global $vzorce;
...
if (ereg("invalid outside limits",$toutput)) 
  {
    $err_msg=__("Invalid limits for integration. Limits for the outside integral must be constants and the lower limit has to be smaller than upper limit."); $problem="vnejsi meze";
  }
...
elseif (ereg("upper and lower limits have intersections",$toutput)) 
  {
    $err_msg=__("Invalid limits for integration. The lower limit for the inside integral has to be smaller than the upper limit."); $problem="spatne vnitrni meze: prusecik";
    $params=sprintf("xmin=%s&xmax=%s&ymin=%s&ymax=%s&a=%s&b=%s&f=%s&g=%s&vars=%s",rawurlencode($xmin),rawurlencode($xmax),rawurlencode($ymin),rawurlencode($ymax),rawurlencode($a),rawurlencode($b),rawurlencode($c),rawurlencode($d),rawurlencode($vars));
    $err_msg=$err_msg."<br><img src=\"../gnuplot/gnuplot_region.php?$params\">";
    if ($vars=="r dr dphi") 
      {
...
if ($err_msg!="")
  {
    ...
    $optjac="";
    if ($vnitrni=="phi") {$vnitrni="\\varphi"; $optjac="r";}
    if ($vnejsi=="phi") {$vnejsi="\\varphi"; $optjac="r";}
    $funkce="";
...
if ($vars=="dx dy") {$swap=1;} else {$swap="";}
if ($vars=="r dr dphi") {$polar=1;} else {$polar="";}
if ($vars=="r dphi dr") {$polar=1; $swap=1;}
...
if  ($vars=="r dr dphi") 
  {
  if ($exprG==" 1 ") {    $jacHTML="r";} else { $jacHTML="\\left( $exprG \\right) r "; }
  }
...
if ($requestedoutput=="pdf")
{
  ...
  fclose($soubor); 
  if (function_exists("doubleint_before_latex")) {doubleint_before_latex();}
...
$outputHTML="  ";
if ($_REQUEST["f1"]=="on") {rundoubleintegration("1","\\iint \\mathrm{d}x\\mathrm{d}y",0);}
if ($_REQUEST["fx"]=="on") {rundoubleintegration("$xmul*($funkce)","\\iint xf(x,y)\\mathrm{d}x\\mathrm{d}y",0);}
if ($_REQUEST["fy"]=="on") {rundoubleintegration("$ymul*($funkce)","\\iint yf(x,y)\\mathrm{d}x\\mathrm{d}y",0);}
if ($_REQUEST["fxx"]=="on") {rundoubleintegration("($xmul)^2*($funkce)","\\iint x^2f(x,y)\\mathrm{d}x\\mathrm{d}y",0);}
if ($_REQUEST["fyy"]=="on") {rundoubleintegration("($ymul)^2*($funkce)","\\iint y^2f(x,y)\\mathrm{d}x\\mathrm{d}y",0);}
